...
Write-Host "Fixing auth imports and configuration..." -ForegroundColor Green

# 1. First, let's force update the config.py
Write-Host "Updating config.py..." -ForegroundColor Yellow
docker-compose exec backend sh -c "cat > /app/app/core/config.py << 'EOF'
...
# 2. Fix the scenarios API to import from the correct location
$fixedScenariosApi = @'
...
from ..services.auth import get_current_active_user  # Fixed import!
...
async def import_scenarios(
    ...
    result = await importer.import_json_file(file, current_user["id"])
    return result
'@
Set-Content -Path "backend\app\api\scenarios.py" -Value $fixedScenariosApi

# 3. Fix sessions API too
$fixedSessionsApi = @'
...
Write-Host "`nFixed! Auth imports and config should now be correct." -ForegroundColor Green
